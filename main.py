...
class GolfCourse:
	...
	def __init__(self,hole_position,ball_position):
		...
		self.ball.displacement.acceleration = vec3d(0,0,0)
	...
	def draw_rect(self, dx, dy):
		...
		self.arrow = pyglet.shapes.Rectangle( width=self.toul, height=5, x=self.x, y=self.y, color=(0, 25, 77),
		                                      batch=self.batch, )
...
def on_mouse_release(x, y, button, modifiers):
	acceleration_vector = vec3d(-(golf_course.ball.displacement.position[0]-x)*10,-(golf_course.ball.displacement.position[0]-x)*10,100)
